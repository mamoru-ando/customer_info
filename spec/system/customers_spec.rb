...
RSpec.describe "お客様詳細", type: :system do
  ...
  it 'ログインしたユーザーはお客様の詳細ページに遷移できる' do
    ...
    expect(page).to have_content("#{@customer.name}")
    expect(page).to have_content("#{@customer.name_kana}")
    expect(page).to have_content("#{@customer.sex_id}")
    expect(page).to have_content("#{@customer.tell1}")
    expect(page).to have_content("#{@customer.address}")
    expect(page).to have_content("#{@customer.memo}")
    expect(page).to have_content("#{@customer.appearance}")
  end
  ...
  it 'ログインしていないとお客様の詳細ページに遷移できない' do
    ...
    visit new_user_session_path
    # 詳細ページのリンクがないことを確認する
    expect(page).to have_no_link "#{@customer.name}", href: customer_path(@customer)
  end
end
